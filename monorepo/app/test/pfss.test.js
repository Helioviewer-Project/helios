...
test("Parse individual pfss file", async () => {
    ...
    let pfss = await ParsePfss(reader);
    expect(pfss.date).toStrictEqual(new Date("2023-10-11T00:14:00.000Z"));
    expect(pfss.lines).toHaveLength(1023);
});
...
test("Parse PFSS Bundle", async () => {
    ...
    expect(pfss_bundle[0].date).toStrictEqual(
        new Date("2023-10-11T00:14:00.000Z")
    );
    expect(pfss_bundle[0].lines).toHaveLength(1023);
    expect(pfss_bundle[1].date).toStrictEqual(
        new Date("2023-10-11T02:14:00.000Z")
    );
    expect(pfss_bundle[1].lines).toHaveLength(1023);
});
