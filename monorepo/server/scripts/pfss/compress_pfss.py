...
def write_bin(json: dict, fname: str):
    n_lines = len(json["lines"])
    data = bytearray()
    data += struct.pack(">Q", json["date"].replace(tzinfo=timezone.utc).timestamp())
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="""Compresses pfss json into a transferrable binary format\n\n

To parse the file, use little endian for all data items.

bytes [0:7]  - unix timestamp representing the observation date for this pfss instance
bytes [8:11] - signed int number of lines
