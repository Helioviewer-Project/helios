try:
    ...
    print(e)
    print(
        "Please run as a module from the parent folder via `python -m scripts.import_pfss`"
    )
    import sys
...
def get_file_list(path: str) -> list:
    ...
    for dirpath, dirname, files in os.walk(path):
        json_files = filter(lambda fname: fname.endswith(".bin"), files)
        file_list += list(
            map(lambda fname: str(Path(f"{dirpath}/{fname}").resolve()), json_files)
        )
    return file_list


def extract_date_from_filename(fname) -> datetime:
    date_string = fname[-24:-4]
...
def add_files_to_db(files: list):
    with Session(engine) as session:
        for filepath in files:
            ...
            already_exists = (
                session.query(GongPFSS).where(GongPFSS.path == filepath).count() == 1
            )
